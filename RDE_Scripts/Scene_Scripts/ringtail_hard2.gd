...
func _ready() -> void:
	set_default_params({"move_torward_player": {"offset": 1, "delay": 0, "speed": 50, "smooth": 100, "length": 1}})
...
func action1() -> void:
	...
	action("move", {"rotate": 0, "speed": 150, "length": 2})
	
	# wait until condition
	action("observe_player", true)
	while player_hp >= 10:
		await GlobalTime.local_wait(0.1)
	action_break()
	
	action("move", {"rotate": -0.5, "speed": 150, "length": 5})
	action("move_torward_player", {"offset": 1, "speed": 150, "length": 1})
...
func action3() -> void:
	#base.hold(true)
	#print("action3")
	action("move_torward_player", {"speed": 100, "length": 0.5})
	...
	for i in range(4):
		action("move_torward_player", {"speed": 25, "length": 1})
		await GlobalTime.local_wait(1)
		shoot()
	controller.chain_action(1)
